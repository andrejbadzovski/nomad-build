      ...
      >
        ...
        <div className="lightbox-image-wrap">
          <AnimatePresence mode="wait">
            <motion.div key={idx} className="lightbox-image" initial={{ opacity: 0, x: 40 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -40 }} transition={{ duration: 0.35, ease: [0.16, 1, 0.3, 1] }}>
        ...
        <div className="lightbox-info">
          ...
          {images.length > 1 && <p className="lightbox-counter">{idx + 1} / {images.length}</p>}
        </div>
      </motion.div>
    </motion.div>,
    document.body
          ...
          return (
            ...
            >
              <div className="project-bg">
                {project.image ? (
                  <Image src={urlFor(project.image).width(900).height(600).url()} alt={project.name} fill sizes="(max-width: 768px) 100vw, 33vw" style={{ objectFit: 'cover' }} />
                ) : (
                  // eslint-disable-next-line @next/next/no-img-element
